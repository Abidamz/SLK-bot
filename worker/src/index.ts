...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecCtxLike): Promise<Response> {
    ...
    if ((url.pathname === "/admin/test-telegram" || url.pathname === "/api/test-telegram" || url.pathname === "/admin/test-loud") && (request.method === "GET" || request.method === "POST")) {
      ...
      const text = [
        "🚨 [LIVE TEST] SLK RADAR NOTIFICATION TEST 🚨",
        "",
        "Your Telegram bot connection is 100% active and healthy.",
        "Delivered with clean standard Telegram push notifications.",
      ...
      try {
        ...
        return json({
          ok: true,
          status: "delivered",
          message: "Standard alert test was successfully sent to your Telegram channel! Check your phone now.",
        });
      } catch (err) {
        return json({
          ok: false,
